...
def build_description_prompt(base_prompt: str, measurement_info: str) -> str:
    ...
    return (
        f"{base_prompt}\n\n"
        f"【商品データ】\n{measurement_info}\n\n"
        "採寸情報がある場合は説明文に自然に含めてください。"
        "採寸情報がない場合でも、画像と商品データから可能な範囲で説明文を作成してください。"
        "出力形式は必ず「タイトル:」「説明文（HTML）:」の2ブロックにしてください。"
    )


def load_measurement_info(bucket, object_name: str) -> tuple[str, bool]:
    """Read _SUCCESS.txt and only treat a successfully read empty body as missing."""
    try:
        measurement_info = bucket.blob(object_name).download_as_text(encoding="utf-8")
    except Exception as error:
        print(f"ERROR: _SUCCESS.txt の読み込みに失敗しました: {error}")
        raise

    if not measurement_info.strip():
        print("WARNING: _SUCCESS.txt は読めましたが本文が空です。採寸情報なしで続行します。")
        return "", False
    print(f"INFO: _SUCCESS.txt を取得しました（文字数: {len(measurement_info)}）。")
